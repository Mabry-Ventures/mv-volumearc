...
| Feature | Status | Notes |
|---------|--------|-------|
| HealthKit depth in coach prompt (HRV / sleep debt / training load) | ✅ | Killer-app differentiator. `RecoveryContext` value type (VOL-145 Phase 1A) + `HealthKitRecoveryReader` (VOL-145 Phase 1B / [VOL-181](https://linear.app/mabry-ventures/issue/VOL-181)) feed `CoachPromptTemplate` with 7-day HRV mean vs 28-day baseline, sleep debt, and weekly strength load. HK-aware intent envelopes for `.recovery` / `.deload` / `.progression` reference the recovery signals when present. `WorkoutDashboardModel` injects the reader and caches the snapshot on every dashboard refresh. |
| Today-tab recovery chip ("What Apple gave us") | ✅ | `VARecoveryChip` surfaces the dominant recovery signal from the cached `RecoveryContext` — HRV trend if `|delta| > 5%`, else sleep debt if `|debt| > 2h`, else Apple Watch Vitals score, else "all systems normal." Tap opens a detail sheet with the full breakdown. Self-suppresses when HK has no data. Shipped in [VOL-181](https://linear.app/mabry-ventures/issue/VOL-181). |
