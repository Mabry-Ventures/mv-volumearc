...
| Co-design planning schedule | In progress | Today `Plan tomorrow` opens the Coach plan draft; `Schedule` persists tomorrow's edited co-designed prescription into the SwiftData weekly plan, refreshes the dashboard, emits `coach.plan_scheduled`, and shows the scheduled card in Workouts with an exercise preview. `Save template` on the plan-draft footer clamps the prescription (coach_template source), persists it as a `WorkoutTemplateRecord` via `SwiftDataWorkoutTemplateRepository` (device-local, V6 schema), and surfaces saved templates in the Workouts idle library for one-tap coach-sourced starts. Scheduling now mirrors the post-clamp plan to the Watch as a `scheduledPlan` WatchConnectivity payload (queued + replayed offline like every watch payload), and the Watch idle screen shows a Tomorrow chip with the first lift's prescription. Dedicated-screen parity remains open under VOL-275; on-Watch start of the mirrored plan ships with the paired-device UAT pass (VOL-271). |
...
| AI review workflow | Done | Requests current-head Codex Code Review. Gemini Code Assist is an independent advisory reviewer when available. |
