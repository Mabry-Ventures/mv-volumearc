...
| 1.5 — Prescription clamps (VOL-284) | Numbers extracted from any coach response are bounded by deterministic code before preview or persistence: load ≤ demonstrated top weight × 1.10 (min one +5 lb step), symptom context freezes load at ≤ 60% of top and RPE ≤ 6, first-exposure caps when an exercise has no history (135 barbell / 50 dumbbell / 75 other; 45/20/45 under symptoms), session volume ≤ largest recent session × 1.20 (sets reduced, never below 1), rest ≥ 90s at ≥ 85% of top weight. Applied at extraction (`clampedCoachWorkoutPlan`) and re-applied at the coach-source scheduling backstop in `scheduleWorkoutPlan` (idempotent). Each intervention emits `coach.safety` / `clamp` telemetry. | `CoachPrescriptionClamp` (pure, `VolumeArcCore/AI`) | `CoachPrescriptionClampTests` (rules, matching, idempotency), `VolumeArcDashboardIntegrationTests.testCoachExtractedPlanClampsInsaneLoadBeforePersisting` |
