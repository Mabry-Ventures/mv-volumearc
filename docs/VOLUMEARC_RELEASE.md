...
| VOL-269 | P0 | App/relay safety hardened locally; release gate added; eval trend still open | AI Coach Excellence | Yes | Publish post-fix green coach response eval trends. |
...
| Coach eval release gate | `scripts/check_coach_eval_trend.sh` now requires the docs and marketing trend files to be mirrored, current within 7 days, and green at 47/47 with no axis or fixture failures. `VOLUMEARC_RELEASE_READY=1 ./scripts/validate_release_config.sh` runs this gate before UAT evidence so release signoff cannot skip the stale/failing public quality record. |
