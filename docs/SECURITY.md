...
|---|---|---|
| `relay.volumearc.app` → Gemini | Coach prompt body (free text the user typed) AND computed HealthKit aggregates (HRV mean, sleep duration, strength load summary, Workout Effort, wrist-temperature trend, respiratory-rate trend) embedded as recovery context in the rendered prompt | Privacy-mode setting controls redaction of the free-text portion (VOL-197 follow-up). Computed HealthKit aggregates flow regardless of privacy mode because they are declared as a tracked data type in `App/PrivacyInfo.xcprivacy`. The relay logs request *shape* only (path / status / latency), never bodies. Gemini's data-handling per its API terms. Raw HealthKit samples are NOT sent — only the derived numeric aggregates produced by `RecoveryContext`. |
