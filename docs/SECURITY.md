...
Nightly response-layer coach evals do not weaken the production path. VOL-244 adds a staging-only eval attestation broker that is refused on `relay.volumearc.app`; it exists so CI can run model-output fixtures with ephemeral P-256 keys, one-time challenges, and monotonic counters without restoring the retired shared HMAC client credential.
