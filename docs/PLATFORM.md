...
GitHub Actions CI runs on ephemeral Tart-backed runners labeled `tart-mv-volumearc` (`runs-on: [self-hosted, tart-mv-volumearc]`). The workflow (`.github/workflows/ci.yml`) triggers on pushes to `main`, pull requests, and version tags (`v*`).
