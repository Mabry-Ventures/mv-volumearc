...
| Marketing site | Scaffold (Wave 1 of Production Readiness) | Next.js 16 + Tailwind CSS v4 + shadcn/ui app under [`marketing/`](../marketing). Adapted from Tailwind Plus Pocket. Pages: `/`, `/terms`, `/privacy`, `/support`, `/quality`. Deploys to Vercel at `volumearc.app`. Shadcnblocks registry mounted (requires `SHADCNBLOCKS_API_KEY` repo secret). CI build/lint/typecheck gate at [`.github/workflows/marketing.yml`](../.github/workflows/marketing.yml). Vercel project link, custom domain DNS, legal-counsel content review, and final design polish remain (VOL-124 + new Vercel deploy ticket). See [`MARKETING.md`](MARKETING.md) for architecture and deploy flow |
