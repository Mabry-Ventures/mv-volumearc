...
| Persistence | Implemented | Four-tier fallback chain, seed data, schema, real repository CRUD all work. `VolumeArcSchemaMigrationPlan` bridges V1 through V6 with lightweight stages and on-disk round-trip migration tests. V6 (VOL-275) adds `WorkoutTemplateRecord` for saved coach templates, stored in a device-local `VolumeArc-LocalOnly` configuration (deliberately not CloudKit-mirrored for v1). App-layer container builders reference the `VolumeArcSchemaLatest` alias, never a literal schema version |
...
| Build pipeline | Implemented | Ruby-generated Xcode project (byte-identical output across regens — VOL-95 / PR #61), CI on the dedicated M4 self-hosted runner label `mv-volumearc-runner` (VOL-88; relabeled from `mv-shared` 2026-06-11), Fastlane local archive fallback, Xcode Cloud, archive script, hard-failing `validate_release_config.sh` on Info.plist keys/URL schemes/BGTask IDs + Release `aps-environment = production` (VOL-70 / PR #43) and on signed entitlements in the built `.xcarchive` — iCloud container identifier, HealthKit, App Groups — via `codesign -d --entitlements -` (VOL-92). The Fastlane `beta` lane runs the validator after `gym` but before `upload_to_testflight` so entitlement regressions fail the local fallback lane before anything hits App Store Connect. Current live TestFlight automation is Xcode Cloud `Internal Testing`, manually run from `main`; no tag-triggered TestFlight workflow exists in App Store Connect today. SwiftLint is pre-flight-checked on the runner and hard-fails on errors (VOL-77 / PR #47). Package.resolved is tracked at repo root as the canonical SPM lockfile (PR #60) |
...
Schema: current `VolumeArcSchemaV6` (exported as `VolumeArcSchemaLatest`) with `VolumeArcSchemaMigrationPlan` bridging `VolumeArcSchemaV1 -> ... -> VolumeArcSchemaV6` via lightweight stages (migration-tested, including a V5 fixture-store round trip).
...
| `scripts/check_coach_eval_trend.sh` | Release-ready gate for response-layer coach quality: docs/marketing trend mirror, no stale record, and 55/55 passing fixtures per cloud tier (flash-lite + pro) with no axis or provider failures |
| `scripts/validate_release_config.sh` | Hard-fails on `ENABLE_TESTABILITY = YES` in Release, `DEBUG_INFORMATION_FORMAT` ≠ `dwarf-with-dsym`, missing entitlements, leaked demo symbols, missing privacy manifests, or unchanged `VERSION` on tag builds; with `VOLUMEARC_RELEASE_READY=1`, also requires current green coach eval trend and complete release UAT evidence |
...
**CI pipeline:** Checkout → pre-flight → project determinism checks → build → unit and integration tests → XCUITest smoke → coverage and SwiftLint → release configuration validation. The separate `.github/workflows/ai-review-gate.yml` requests and observes Codex Code Review for the current PR head. Gemini Code Assist can provide independent advisory feedback.
