...
└── VolumeArcScreenshotTests.swift                # fastlane/manual App Store screenshots
...
| `accessibility-screenshots` | `VolumeArcAccessibilityJourneyTests`                                                                                                                            | Known AX-daemon wedge cause (`accessibility5` + `NSDoubleLocalizedStrings`); when the daemon dies, the wedge stays inside this shard |

The shard map lives in `scripts/test_apple_targets.sh` (`UI_SHARDS` + `ui_shard_classes`). A `verify_shard_coverage` gate at the top of the UI test block fails the build if you add a new `XCTestCase` class and forget to assign it to a shard or the explicit manual-only allowlist — so a forgotten class can't silently stop running in CI.

`VolumeArcScreenshotTests` is intentionally manual-only. It remains compiled in the UI test target for `fastlane ios screenshots`, but ordinary CI does not run it because screenshot capture is a release-lane artifact and the test skips unless `VOLUMEARC_RUN_SCREENSHOT_CAPTURE=1` is set.
...
| `DebuggerLLDB.DebuggerVersionStore.StoreError` | LLDB's runner-side debug-info index is stale after a simulator/Xcode update | The runner-crash retry clears only `~/Library/Developer/Xcode/LLDB/VersionStore` after this signature appears, then reboots + retries the shard. Routine warm-up leaves debugger state alone. | VOL-227 follow-up in `scripts/test_apple_targets.sh::clear_lldb_version_store` |
