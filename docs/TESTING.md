...
- **Disk-full** — module cache writes start failing, which cascades into simulator boot wedges + xctest bootstrap failures. Symptom: `'No space left on device'` in the build log. Fix: clean the four directories listed in the taxonomy above; investigate why the cache grew unboundedly if recurring. **Prevention** tracked in [VOL-243](https://linear.app/mabry-ventures/issue/VOL-243) — daily runner-host disk-pressure watchdog at a 50 GB free-disk floor, well above the 10 GB Pre-flight hard floor, so the operator gets warning before a build hits the cliff.
