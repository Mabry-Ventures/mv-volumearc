...
| `DebuggerLLDB.DebuggerVersionStore.StoreError` | LLDB's runner-side debug-info index is stale after a simulator/Xcode update | `warm_simulator_for_tests` clears only `~/Library/Developer/Xcode/LLDB/VersionStore`; the normal runner-crash retry then gets a clean simulator + fresh VersionStore | VOL-227 follow-up in `scripts/test_apple_targets.sh::warm_simulator_for_tests` |
